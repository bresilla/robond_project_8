...
namespace gazebo
{
	...
	bool ArmPlugin::createAgent(){
		...
		agent = dqnAgent::Create(INPUT_WIDTH, INPUT_HEIGHT, INPUT_CHANNELS, INPUT_CHANNELS * 2,
	...
	void ArmPlugin::onCollisionMsg(ConstContactsPtr &contacts){
		...
		for (unsigned int i = 0; i < contacts->contact_size(); ++i)
		{
			...
			if( !prop )	{printf("ArmPlugin - failed to find Prop '%s'\n", PROP_NAME); return;}
			physics::LinkPtr gripper  = model->GetLink(GRIP_NAME);		
			if( !gripper ) {printf("ArmPlugin - failed to find Gripper '%s'\n", GRIP_NAME); return;}
			const float distGoal = BoxDistance(prop->model->GetBoundingBox(), gripper->GetBoundingBox());

		
			// TODO: COLLISIONS		
			bool collisionTube = (strcmp(contacts->contact(i).collision1().c_str(), COLLISION_ITEM) == 0);
			bool collisionGrip = (strcmp(contacts->contact(i).collision2().c_str(), COLLISION_POINT) == 0);
	...
	void ArmPlugin::OnUpdate(const common::UpdateInfo& updateInfo){
		...
		if( !agent && updateInfo.simTime.Float() > 1.5f ){if( !createAgent() ) return;}
		...
		// get the bounding box for the prop object and the gripper
		PropPlugin* prop = GetPropByName(PROP_NAME);		
		if( !prop )	{printf("ArmPlugin - failed to find Prop '%s'\n", PROP_NAME); return;}
		...
		if( maxEpisodeLength > 0 && episodeFrames > maxEpisodeLength ){  
			// TODO - set episode end reward
			printf("ArmPlugin - triggering EOE, exceeded %i frames\n", maxEpisodeLength);
			rewardHistory = 1000 * REWARD_LOSS + abs(100 * distGoal);
			newReward     = true;
			endEpisode    = true;
		}
		...
		if( hadNewState && !newReward ){
			...
			if(!checkGroundContact)	{
				...
				if( episodeFrames > 1 )
				{
					...
					rewardHistory = 4.0f * avgGoalDelta - 0.3f;
